...
class TestInstructionModifier(TestCase):
    ...
    def test_modify_rn(self):
        inst = CAPSInstruction('000209b4', 8, little_endian=False)  # strheq;r0, [r2], -r4
        mod = InstructionModifier()
        self.assertEqual('strheq\tr0, [r0], -r4', str(CAPSInstruction(mod.modify_rn(inst, 0), 8)))
        self.assertEqual('strheq\tr0, [r4], -r4', str(CAPSInstruction(mod.modify_rn(inst, 4), 8)))
        self.assertEqual('strheq\tr0, [sl], -r4', str(CAPSInstruction(mod.modify_rn(inst, 10), 8)))
